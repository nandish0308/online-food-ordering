...
export const getAllRestaurantById = (reqData) => {
    return async (dispatch) => {
        ...
        try {
            ...
            dispatch({ type: GET_RESTAURANT_BY_ID_FAILURE, payload: error.message }); // ✅ Send only serializable error
        }
    };
};
...
export const getAllEvents = ( jwt ) =>{
    return async (dispatch) => {
       ...
       console.log("get all events events",res.data)
    } catch (error) {
        console.log(" catch -",error);
...
export const deleteEventAction = ({eventId, jwt }) =>{
    return async (dispatch) => {
        ...
        try {
            const res = await Api.delete(`/api/admim/events/${eventId}`,
                data, {
                headers: {
                    Authorization: `Bearer ${jwt}`,
            },
